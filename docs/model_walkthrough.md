...
**决策规则**:
- |z| 正常 → 照常 1W roll(灵活)。
- z 极端且方向使 1W roll 变贵 → **锁 1M**(被动版: 只是省 funding 成本)。
- 主动 RV(上面例②): 进 1M swap + 1W 反向 roll = 纯赌 basis 收敛,
  独立的 **funding RV 子账本**, 与主书 spot 方向正交。
**状态**: `L2_fwdpts_basis_1w_1m` + z 已进管线; `funding_rv.py` 做每货币
收敛性验证(离异后 1–4 周 basis 是否回落、例②交易的命中率)。
