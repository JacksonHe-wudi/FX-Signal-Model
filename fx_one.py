...
FX ONE - the whole system in a single file.
    ...
    python3 fx_one.py                      # auto-finds FX_Model_Data*.xlsx in
                                           # this folder, runs the FULL data
                                           # pipeline (all dashboard tables +
                                           # pretty workbook), then the
                                           # backtest, charts and order sheet
    python3 fx_one.py <workbook.xlsx>      # same, explicit path
    python3 fx_one.py --clean out/clean.csv.gz   # re-run without the xlsx
    python3 fx_one.py fetch [cftc bis fred imf equity fxspot]  # free external data
    python3 fx_one.py research             # ML walk-forward, PCA drivers,
                                           # strategy library, STFV (feeds the
                                           # ML Lab / Drivers & Meta tabs)
    streamlit run fx_one.py                # the dashboard (7 tabs)

One command produces everything: running the CLI on the workbook writes
data/clean/clean_csv/ (the 58 cleaned tables), data/clean/FX_Model_Clean.xlsx
(the pretty workbook), the backtest outputs under out/, and the weekly order
sheet - after which `streamlit run fx_one.py` has all the data it needs.

Contents: PART 0 the full data pipeline (Excel -> 58 tables, completeness
gate, pretty workbook) · PART 1 the model (three sleeves, backtest with
carry/spot attribution and implementation-noise ensemble, order sheet) ·
PART 2 free external data fetchers (CFTC/BIS/FRED/IMF/Yahoo) · PART 3 the
Velocity-styled dashboard (entry verdicts, currency deep-dive, strategy
backtester, ML lab, PCA drivers + strategy leaderboard with the
anti-performance-chasing verdict, news, all-currency fair-value grid).

Optional research artifacts (analysis/ml_predictions.csv, pca csvs, STFV)
enrich the ML and Drivers tabs when present; those tabs degrade gracefully
with instructions when absent.

Dependencies: pandas numpy openpyxl matplotlib · dashboard: streamlit plotly
· news tab: feedparser · ML tab: scikit-learn.
"""
...
# %% PART 0 - FULL DATA PIPELINE (Excel -> 58 tables, in its own namespace) %%
...
def _pipeline_ns():
 ...
 return {'build': build, 'write_outputs': write_outputs,
...
# %% PART 2 - FREE EXTERNAL DATA FETCHERS %%
...
# %% PART 2.5 - RESEARCH PRECOMPUTES (fx_one.py research) %%
...
if _streamlit_ctx():
    ...
    elif len(_sys.argv) > 1 and _sys.argv[1] == 'research':
        research_main()
    else:
        cli_main()
